...
void km_find_match(const char *fn, const kmerdb_t *db)
{
	...
	const size_t flush_threshold = 65536; // Flush when buffer reaches 64KB
	fp = fn && strcmp(fn, "-")? gzopen(fn, "rb") : gzdopen(0, "rb");
	if (fp == 0) return;
	ks = kseq_init(fp);
	while (kseq_read(ks) >= 0) {
		...
		for (i = l = 0; i < ks->seq.l; ++i) {
			int c = kom_nt4_table[(uint8_t)ks->seq.s[i]];
			if (likely(c < 4)) { // Most nucleotides are valid (A, C, G, T)
				xf = (xf << 2 | c) & mask;
				...
				if (++l >= db->ksize) {
					...
					if (unlikely(out.l > flush_threshold)) {
